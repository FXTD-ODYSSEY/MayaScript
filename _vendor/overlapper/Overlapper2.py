...
def overlap():
    ...
    for i,[controller,jnt] in enumerate(zip(controller_list,jnt_list)):
        if NotUseFirstCtrl and i == 0:
            continue
        if cmds.getAttr(controller+".t",q=1,k=1) and not cmds.getAttr(controller+".t",q=1,l=1):
            constraint_list.extend(cmds.pointConstraint(jnt,controller,mo=1))
        if cmds.getAttr(controller+".r",q=1,k=1) and not cmds.getAttr(controller+".r",q=1,l=1):
