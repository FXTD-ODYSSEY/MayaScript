...
class MatrixNN(object):
    ...
    def inverse(self, A):
        ...
        if d == 0:
            ...
            B = self.multCoef(self.comat(A), 1.0 / d)
            inv = self.transpos(B)
        return inv
...
class RbfSolver(OpenMayaMPx.MPxNode):
    ...
    def nodeInitializer(cls):
        ...
        cls.NDimension = numAttr.create(
            "NDimension", "nd", OpenMaya.MFnNumericData.kInt, 1
        )
        numAttr.setMin(1)
        cls.MDimension = numAttr.create(
            "MDimension", "md", OpenMaya.MFnNumericData.kInt, 1
        )
        numAttr.setMin(1)
        ...
        cls.normalize = numAttr.create(
            "normalize", "nz", OpenMaya.MFnNumericData.kBoolean, True
        )

        # force output in 0-1 range
        cls.blendShapeMode = numAttr.create(
            "blendShapeMode", "bsm", OpenMaya.MFnNumericData.kBoolean, False
        )
        ...
        cls.state = numAttr.create(
            "state", "st", OpenMaya.MFnNumericData.kBoolean, True
        )
        ...
        # out
        cls.solved = numAttr.create(
            "solved", "sv", OpenMaya.MFnNumericData.kBoolean, True
        )
        numAttr.setHidden(True)

        cls.mOutput = numAttr.create(
            "mOutput", "mo", OpenMaya.MFnNumericData.kFloat, 0.0
        )
    ...
    def compute(self, plug, data):
        ...
        if blendShapeMode == True:
            ...
            sum = 0.0
            for m in xrange(self.M):
                outputs[m] = self.set_range(outputs[m], min, 1.0, 0.0, 1.0)
                sum += outputs[m]
...
def initializePlugin(mobject):
    mplugin = OpenMayaMPx.MFnPlugin(
        mobject, "Hans Godard -- zapan669@hotmail.com", "1.0", "Any"
    )
    try:
        mplugin.registerNode(
            RbfSolver.kPluginNodeName,
            RbfSolver.kPluginNodeId,
            RbfSolver.nodeCreator,
            RbfSolver.nodeInitializer,
        )
    except:
